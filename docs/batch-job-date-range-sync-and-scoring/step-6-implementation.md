...
- `backend/src/modules/jira-crawler/batch-job.scheduler.ts`:
  - Khắc phục triệt để lỗi nhân đôi bản ghi đi trễ (duplicate attendance records): áp dụng `DISTINCT ON (year_month, target_member)` trên SQL query bảng `collector_monthly_snapshot` để bảo toàn dữ liệu chấm công của tất cả các bộ phận (`Maritime Solutions Part`, `ALLEGRO NX Part`), tránh việc snapshot sau ghi đè làm mất 5 nhân sự (Phạm Mai Nhật, Lê Minh Hy, Nguyễn Bá Ngọc, Nguyễn Thành Phước, Phan Huy Nhân) dẫn đến hiển thị "Chỉ Jira".
  - Bổ sung cơ chế khử trùng lặp trong bộ nhớ `seenEmpDates = new Set<string>()` theo `${empCode}#${dateStr}`. Tổng số ngày công của Thái Thanh Xuân chuẩn hóa từ 90 ngày xuống 76 ngày, số bản ghi trễ giảm từ 2 bản ghi trùng lặp xuống đúng 1 bản ghi (ngày Sep-11-2026, 6 phút trễ).
