...
export const calibrationApi = {
  ...
  getDistribution: async (sessionId: string): Promise<CalibrationDistribution> => {
    return getApi<CalibrationDistribution>(`/api/calibration-sessions/${encodeURIComponent(sessionId)}/distribution`);
  },

  getAdjustments: async (sessionId: string): Promise<CalibrationAdjustment[]> => {
    return getApi<CalibrationAdjustment[]>(`/api/calibration-sessions/${encodeURIComponent(sessionId)}/adjustments`);
  },
