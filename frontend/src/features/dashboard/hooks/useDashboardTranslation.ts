...
import { DASHBOARD_FALLBACK_TRANSLATIONS } from '../i18n/dashboard-translations';
...
export function useDashboardTranslation() {
  ...
  const t = useCallback(
    (key: string, fallback?: string): string => {
      ...
      const dict = DASHBOARD_FALLBACK_TRANSLATIONS[targetLocale];
      ...
      if (targetLocale !== 'en') {
        const enDict = DASHBOARD_FALLBACK_TRANSLATIONS.en;
