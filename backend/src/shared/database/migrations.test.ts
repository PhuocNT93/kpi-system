...
if (canRunMigrationTests) {
  ...
  it('creates the LLD-aligned department schema', async () => {
    ...
    expect(rows.map((column) => column.column_name)).toEqual([
      'department_id',
      'code',
      'name',
      'active',
    ...
    expect(rows.find((column) => column.column_name === 'department_id')).toMatchObject({
      data_type: 'uuid',
      column_default: expect.stringContaining('gen_random_uuid')
    });
    expect(rows.filter((column) => column.column_name.endsWith('_at')).every((column) => column.data_type === 'timestamp with time zone')).toBe(true);
  });
